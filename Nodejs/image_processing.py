...
img,box,conf=face.face_detection(image_path= path + save_name ,model='tiny')

for (x, y, w, h) in box: 
    faces = img[y:y + w, x- 30:x + h + 30]

resized_image = cv2.resize(faces, (256, 256))
cv2.imwrite('C:\\git\\FRA_503-IOT\\Nodejs\\Processed_image\\'+sys.argv[1]+'step1.png', resized_image) 
...
def show_plot(imagesX, imagesY1, imagesY2):
    ...
    images = (images + 1) / 2.0
    matplotlib.image.imsave('C:\\git\\FRA_503-IOT\\Nodejs\\Processed_image\\'+sys.argv[1]+'step2.png', images[1])
...
dataB_all = load_images('C:\\git\\FRA_503-IOT\\Nodejs\\Processed_image\\'+sys.argv[1]+'step1.png')
...
img = cv2.imread('C:\\git\\FRA_503-IOT\\Nodejs\\Processed_image\\'+sys.argv[1]+'step2.png',0)
binr = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1]
kernel = np.ones((2,2), np.uint8)
opening = cv2.morphologyEx(binr, cv2.MORPH_OPEN, kernel, iterations=1)  
open_n_closing = cv2.morphologyEx(opening, cv2.MORPH_CLOSE, kernel, iterations=1) 
matplotlib.image.imsave('C:\\git\\FRA_503-IOT\\Nodejs\\Processed_image\\'+sys.argv[1]+'step3.png', open_n_closing)


###########Step 4 Image Contour###############
img2 = cv2.imread('C:\\git\\FRA_503-IOT\\Nodejs\\Processed_image\\'+sys.argv[1]+'step3.png')
...
cv2.imwrite('C:\\git\\FRA_503-IOT\\Nodejs\\Processed_image\\'+sys.argv[1]+'step4.png', 255-line_image)
